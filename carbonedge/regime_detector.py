...
# ---------------------------------------------------------------------------
# Backtest: run detectors over historical data and report detections
# ---------------------------------------------------------------------------
...
def backtest_regime_detection(
    ...
    """Replay detectors over historical price series. Returns status at each tick."""
    monitor = RegimeMonitor(focus_config, cusum_config)
    results = []
    for i, price in enumerate(prices):
        fc = forecasts[i] if forecasts and i < len(forecasts) else None
        status = monitor.update(price, fc)
        results.append(status)
    return results


def backtest_summary(statuses: List[RegimeStatus]) -> Dict:
    """Summarize backtest results."""
    focus_detections = [s for s in statuses if s.focus_triggered]
    cusum_detections = [s for s in statuses if s.cusum_triggered]
    regime_changes = [s for s in statuses if s.regime_change]

    return {
        "total_ticks": len(statuses),
        "focus_detections": len(focus_detections),
        "focus_detection_ticks": [s.tick for s in focus_detections],
        "cusum_detections": len(cusum_detections),
        "cusum_detection_ticks": [s.tick for s in cusum_detections],
        "regime_changes": len(regime_changes),
        "regime_change_ticks": [s.tick for s in regime_changes],
        "regime_change_details": [
            {"tick": s.tick, "message": s.cusum_message}
            for s in regime_changes
        ],
    }
